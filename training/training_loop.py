...
def train(model, loader, optim, lr, warmup_steps, wd, device='cuda',
          run_dir='checkpoints', hparams=None):
    model.train()
    ...
    def ckpt_meta():
        # stamped into every checkpoint so any .pt is self-describing
        return {"hparams": hparams, "step": step,
    ...
    def snapshot(reason, attribute):
        ...
        if step - last_alarm < ALARM_COOLDOWN:
            return
        last_alarm = step
        path = save_checkpoint(model, f"{run_dir}/rjepa_ALARM_step{step}_{reason}.pt",
                               optim=optim, epoch=0, meta=ckpt_meta())
        print(f"  !! ALARM [{reason}] step={step} -> snapshot {path}")
        if attribute:
            # rank every signal by when it first left its baseline; earliest = root.
            explain_collapse(armed_hist(), log_path=f"{run_dir}/rjepa_ALARM_step{step}_{reason}.jsonl")
        ...
        for epoch in range(EPOCHS):
            ...
            for window in loader:
                ...
                if step and step % 5_000 == 0:
                    save_checkpoint(model, f"{run_dir}/rjepa_latest.pt", optim=optim, epoch=epoch,
                                    meta=ckpt_meta())
                ...
                if step and (step in _tagged or (step > 50_000 and step % 10_000 == 0)):
                    save_checkpoint(model, f"{run_dir}/rjepa_step{step // 1000}k.pt", meta=ckpt_meta())
                        ...
                        if not armed:
                            ...
                            if not reported and collapse:
                                ...
                                explain_collapse(armed_hist(),
                                                 log_path=f"{run_dir}/rjepa_CONFIRMED_step{step}.jsonl")
                                reported = True

                step += 1
    except KeyboardInterrupt:
        path = save_checkpoint(model, f"{run_dir}/rjepa_interrupt_step{step}.pt",
                               optim=optim, epoch=epoch, meta=ckpt_meta())
        print(f"\nInterrupted at step {step} "
              f"({(time.time() - t_start) / 3600:.2f}h in) — saved checkpoint to {path}")
        raise
